#if defined(CONFIGT) && defined(EWALD)
/* ======================================================================== */
/* ewald.cpp                                                                */
/*                                                                          */
...
/*                                                                          */
/* ************************************************************************ */
void ewald_kspace2 (int ibox, double *enkewald)
{
  /* ================================================================== */
  /*                                                                    */
	...
	for(int kz=-kmax_z; kz<=kmax_z; kz++){
		...
		if(ksq<=max_k_square && ksq!=0){
			...
			for(int i=0; i<ns; i++){
				...
				config[k].hesy += hes*ibly*ibly*(double)ky*(double)ky;
				config[k].hesz += hes*iblz*iblz*(double)kz*(double)kz;
			}
	  ...
	  for(int ky=-kmax_y; ky<=kmax_y; ky++){
		  for(int kz=-kmax_z; kz<=kmax_z; kz++){
			...
			for(int i=0; i<ns; i++){
				...
				config[k].hesx += hes*iblx*iblx*(double)kx*(double)kx;
				config[k].hesy += hes*ibly*ibly*(double)ky*(double)ky;
				config[k].hesz += hes*iblz*iblz*(double)kz*(double)kz;
...
/* ************************************************************************ */
double erfc(double);
void ewald_rspace2 (int ibox, double *energy)
{
  ...
  /* ----------------------------------------------- */
	  ...
	  if (test > 0){
		...
		config[k].hesx += 2.0*qq12 * (hes1 - dx*dx*hes2);
		config[k].hesy += 2.0*qq12 * (hes1 - dy*dy*hes2);
		config[k].hesz += 2.0*qq12 * (hes1 - dz*dz*hes2);
