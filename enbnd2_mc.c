...
void enbnd2_mc(int lb, int ub, int ibox, double *energy)
{
  ...
  /* ----------------------------------------------- */
	  ...
	  hesor += 2.0*eps*(528.0*sr12-120*sr6)/dr2;
#endif
	  }
  /* ----------------------------------------------- */
  ...
  /* ----------------------------------------------- */
	...
	if (qq!=0.0) {
			  ...
			  ecoulomb  += qq * (dri + rfc * dr2) - qq * (1.0 + rfcs) / rc;
					...
					hesox += 2.0*(term4*dx2-term3);
					hesoy += 2.0*(term4*dy2-term3);
					hesoz += 2.0*(term4*dz2-term3);
				#endif	//RDIE
	...
	int test = ljset[k].pot[a][b].bond_flag + ljset[k].pot[a][b].bend_flag + ljset[k].pot[a][b].tors_flag + ljset[k].pot[a][b].exNB_flag;
      ...
      if (test > 0){
        ...
        eewald_intra += qqab*(1.0-erfckr)*dri;
		    ...
		    hesox += 2.0*qqab * (hes1 - dx*dx*hes2);
		    hesoy += 2.0*qqab * (hes1 - dy*dy*hes2);
		    hesoz += 2.0*qqab * (hes1 - dz*dz*hes2);
      }
#endif
#endif
  ...
  double enkewald[3];
#ifdef SPME
  pme_calc2_mc(k,enkewald);
#else
  ewald_kspace2_mc(k,enkewald);
#endif
  /* ------------------------------------------ */
...
#else
  ewald_rspace2_mc(lb,ub,k,enrewald);
#endif  
  /* ------------------------------------------ */
