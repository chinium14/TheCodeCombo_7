...
#else
void ewald_kspace (int,double*);
#endif
void ewald_rspace (int,double*);
double erfc(double);
#endif
#ifdef PR_NPT
...
void cnonbondnl(int ibox, double *energy)
{

  int k = ibox;
  //double fconv = (ELEC * ELEQ * ELEQ * NA * 0.001);   // convert atomic charges to units of kJ/mol (fconv is not used)
  /* ================================================================== */
  /*                                                                    */
  ...
  #if defined(COULOMB) && defined(SHIFTF)
...
//#endif
  ...
  /* ----------------------------------------------- */
      ...
      qq = pott[k][a].qq * pott[k][b].qq * QFACTOR;
  ...
  /* ----------------------------------------------- */
      ...
      if((eps*sig)!=0.0) {	  
        ...
        #ifdef BGO //Brooks Go model
        //if(id == -10){//not a native contact
        //  force = 12.0*eps/dr2*sr12;
        //  enonbond  += eps * sr12;
        //  enonbonds += eps * sr12s;
        //}else{//native contact
          double sr10 = sr6 * sr2 * sr2;
          double sr10s = sr6s * sr2s * sr2s;
          force = eps/dr2*(156.0*sr12 - 180.0*sr10 + 24.0*sr6);
          //enative += eps*(13.0*sr12 - 18.0*sr10 + 4.0*sr6);
          //enatives += eps*(13.0*sr12s - 18.0*sr10s + 4.0*sr6s);
...
//		double qqab = pott[k][a].qq * pott[k][b].qq * fconv;
		double qqab = pott[k][a].qq * pott[k][b].qq * QFACTOR;
...
//#ifdef BGO
//  energy[0] = energy[0] + enative;
//  energy[2] = enative - enatives;   //shifted native
//  energy[1] = energy[1] + energy[2]; //shifted native + nonnative
