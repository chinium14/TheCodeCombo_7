...
int swap_ct_dos(unsigned long iter, int ibox)
{
	int k=ibox;
...
*/
	...
	/* ----------------------------------------	*/
	for(int i=0; i<o_bins; i++){
		dos_hist[k][j+i].ct_num =  dos_hist[k][j+i].ct_num + dos_hist[k+1][i].ct_num;
		dos_hist[k][j+i].ct_den =  dos_hist[k][j+i].ct_den + dos_hist[k+1][i].ct_den;
		dos_hist[k+1][i].ct_num =  dos_hist[k][j+i].ct_num;
		dos_hist[k+1][i].ct_den =  dos_hist[k][j+i].ct_den;
	}
}
#endif
#endif
