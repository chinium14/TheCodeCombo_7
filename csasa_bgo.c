...
double csasa (int ibox)
{
  ...
  /* ================================================================== */
#ifdef MEMBRANE
//   double *concent_local = (double *) calloc(box[k].boxns,sizeof(double));
#endif
...
#ifdef MEMBRANE
  #ifndef SASAREX
    concent_local[a] = concentration[k] * exp(-(az*az)/30.0);
  //  printf("concent_local[%d] = %lf\n",a, concent_local[a]);
  #else
    concent_local[a] = concentration[mpi.my_rank] * exp(-(az*az)/30.0);
  #endif

  #ifdef MEMINSIDE
     for (int i=0; i< number_in; i++)
     {
        if (a == mem_in_list[i]) concent_local[a] = 0; 
     }
  #endif
#endif
...
#ifndef MEMBRANE
#ifndef SASAREX
    double sigA = scale_factor*((mBB + mSC) * concentration[k] + scale_factor_b*(bBB + bSC)) * (sasa[k][a].A / (sBB + sSC));
#else
    double sigA = scale_factor*((mBB + mSC) * concentration[mpi.my_rank] + scale_factor_b*(bBB + bSC)) * (sasa[k][a].A / (sBB + sSC));
#endif
#else
    double sigA = scale_factor*((mBB + mSC) * concent_local[a] + scale_factor_b*(bBB + bSC)) * (sasa[k][a].A / (sBB + sSC));
#endif
...
#ifndef MEMBRANE
#ifndef SASAREX
          double sigB_const = scale_factor*((mBBb + mSCb) * concentration[k] + scale_factor_b*(bBBb + bSCb)) * (1.0 / (sBBb + sSCb));
#else
          double sigB_const = scale_factor*((mBBb + mSCb) * concentration[mpi.my_rank] + scale_factor_b*(bBBb + bSCb)) * (1.0 / (sBBb + sSCb));
#endif
#else
          double sigB_const = scale_factor*((mBBb + mSCb) * concent_local[a] + scale_factor_b*(bBBb + bSCb)) * (1.0 / (sBBb + sSCb));
#endif
...
#ifndef SASAREX
      sigW = scale_factor*(mSCbw*concentration[k] + scale_factor_b*bSCbw) * ((1-pbw*pabw*PI*((rbw+rpw)*(rbw+rpw)-(dzw-rpw)*(dzw-rpw))/((sBB+sSC)/4.0)));
      double forcewa = scale_factor*((mBB + mSC) * concentration[k] + scale_factor_b*(bBB + bSC)) * 
                                    ((-(paw*pabw*2*PI*(raw+rpw))/(sBB+sSC)))+
                       scale_factor*(mSCbw * concentration[k] + scale_factor_b*bSCbw) *
                                 (-2*pbw*pabw*PI*(abs(dzw-rpw))/((sBB+sSC)/4.0));
#else 
      sigW = scale_factor*(mSCbw*concentration[mpi.my_rank] + scale_factor_b*bSCbw) * ((1-pbw*pabw*PI*((rbw+rpw)*(rbw+rpw)-(dzw-rpw)*(dzw-rpw))/((sBB+sSC)/4.0)));
      double forcewa = scale_factor*((mBB + mSC) * concentration[mpi.my_rank] + scale_factor_b*(bBB + bSC)) * 
                                    ((-(paw*pabw*2*PI*(raw+rpw))/(sBB+sSC)))+
                       scale_factor*(mSCbw * concentration[mpi.my_rank] + scale_factor_b*bSCbw) *
                                 (-2*pbw*pabw*PI*(abs(dzw-rpw))/((sBB+sSC)/4.0));
#endif
...
*/
  ...
  /* ----------------------------------------- */
    ...
    ff[k][a].y += fya;
#ifdef MEMBRANE
      fza += scale_factor*((mBB + mSC) * concent_local[a] *(az/15.0))* (sasa[k][a].A / (sBB + sSC));
#endif
    ff[k][a].z += fza;
    esasa      += sigA;
#ifdef WWALL
...
#ifdef MEMBRANE
#ifndef SASAREX
    concent_local[box[k].boxns-1] = concentration[k] * exp(-(atom[k][box[k].boxns-1].z*atom[k][box[k].boxns-1].z)/30.0);
#else
    concent_local[box[k].boxns-1] = concentration[mpi.my_rank] * exp(-(atom[k][box[k].boxns-1].z*atom[k][box[k].boxns-1].z)/30.0);
#endif
#endif
...
#ifndef MEMBRANE
#ifndef SASAREX
    esasa += scale_factor*((mBB + mSC) * concentration[k] + scale_factor_b*(bBB + bSC)) * (sasa[k][box[k].boxns-1].A / (sBB + sSC));
#else
    esasa += scale_factor*((mBB + mSC) * concentration[mpi.my_rank] + scale_factor_b*(bBB + bSC)) * (sasa[k][box[k].boxns-1].A / (sBB + sSC));
#endif
#else
    esasa += scale_factor*((mBB + mSC) * concent_local[box[k].boxns-1] + scale_factor_b*(bBB + bSC)) * (sasa[k][box[k].boxns-1].A / (sBB + sSC));
#endif
  /* ================================================================== */
  /*                                                                    */
